...
{
    public class Doran_Relatorio_Retorno : IDisposable
    {
        ...
        public Doran_Relatorio_Retorno(decimal _ID_EMPRESA)
        {
            ID_EMPRESA = _ID_EMPRESA;
        }

        public string MontaRelatorioRetorno()
        {
            string retorno = "";
            ...
            using (Th2_Report r = new Th2_Report(ExpertPdf.HtmlToPdf.PdfPageSize.A4, ExpertPdf.HtmlToPdf.PDFPageOrientation.Portrait))
            {
                ...
                using (Doran_ERP_Servicos_DadosDataContext ctx = new Doran_ERP_Servicos_DadosDataContext())
                {
                    ...
                    string cabecalho = string.Format("Relat&oacute;rio Retorno Banc&aacute;rio por Emiss&atilde;o<br /><span style='font-family: Tahoma; font-size: 8pt;'>Per&iacute;odo de {0} at&eacute; {1}<br />{2}</span>",
                    ...
                    _conteudo.Append(@"<tr>
                                    ...
                                    <td style='BORDER-BOTTOM: 1px solid; border-color:#C0C0C0;'>Ocorr&ecirc;ncia de Retorno</td>
                                  </tr>");
                    ...
                    var query = from linha in ctx.TB_HISTORICO_CNABs
                                ...
                                select new
                                {
                                    ...
                                    DESCRICAO_OCORRENCIA = 
                                        (from linha1 in ctx.TB_OCORRENCIA_BANCARIA_RETORNOs
                                         where linha1.COD_OCORRENCIA == linha.INSTRUCAO
                                         select linha1).Any() ?

                                         (from linha1 in ctx.TB_OCORRENCIA_BANCARIA_RETORNOs
                                          where linha1.COD_OCORRENCIA == linha.INSTRUCAO
                                          select linha1.DESCRICAO_OCORRENCIA).First() : ""
                                };
                    ...
                    retorno = r.SalvaDocumento("Doran_Retorno_Bancario");
                }
