function Matriz_Orcamento() {
    ...
    var TB_MATRIZ_ORCAMENTO_TP = new Ext.TabPanel({
    ...
    var buttonGroup_MATRIZ = new Ext.ButtonGroup({
        items: [{
            ...
            handler: function () {
                ...
                _ajax.setUrl(TB_MATRIZ_ORCAMENTO_TP.activeTab.title == 'Formato da Primeira P&aacute;gina' ?
                        'servicos/TB_ORCAMENTO_VENDA.asmx/Salva_Matriz_Primeira_Pagina' :
                        'servicos/TB_ORCAMENTO_VENDA.asmx/Salva_Matriz_Proximas_Paginas');

                _ajax.setJsonData({
                    HTML: TB_MATRIZ_ORCAMENTO_TP.activeTab.title == 'Formato da Primeira P&aacute;gina' ?
    ...
    var formMatrizOrcamento = new Ext.form.FormPanel({
        labelAlign: 'top',
        bodyStyle: 'padding:5px 2px 0px 0px',
        frame: true,
        width: '100%',
        autoHeight: true,
        items: [TB_MATRIZ_ORCAMENTO_TP]
    });

    var panelMatrizOrcamento = new Ext.Panel({
        width: '100%',
        border: true,
        title: 'Matriz de Impress&atilde;o de Or&ccedil;amentos',
        items: [formMatrizOrcamento, toolbar_MATRIZ]
    });
    ...
    function Busca_Primeira_Pagina() {
        var _ajax = new Th2_Ajax();
        _ajax.setUrl('servicos/TB_ORCAMENTO_VENDA.asmx/Matriz_Primeira_Pagina');
    ...
    function Busca_Proximas_Paginas() {
        var _ajax = new Th2_Ajax();
        _ajax.setUrl('servicos/TB_ORCAMENTO_VENDA.asmx/Matriz_Proximas_Paginas');
        _ajax.setJsonData({ ID_USUARIO: _ID_USUARIO });
